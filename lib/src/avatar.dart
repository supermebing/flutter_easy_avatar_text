...
class Avatar extends StatelessWidget {
  ...
  String _getDisplayStr() {
    ...
    // 如果是中文，则根据字符数量返回相应的字符串
    if (RegExp(r'[\u4e00-\u9fa5]').hasMatch(str)) {
      if (str.length == 2) {
        return str;
      } else if (str.length >= 3) {
        return str.substring(str.length - 2);
      }
      return str; // 处理长度为1的情况
    }
    ...
    if (charList.length > 1 && charList.length == wordsCount) {
      ...
      return '${charList[charList.length - 2][0].trim()}${charList[charList.length - 1][0].trim()}';
    }
    if (isConsole) {
  ...
  Color _getBgColor() {
    if (backgroundColor != null) {
      ...
      String displayStr = _getDisplayStr();
      if (RegExp(r'[\u4e00-\u9fa5]').hasMatch(displayStr)) {
        // 中文字符，可以根据其编码生成随机颜色或者使用预定义的颜色
        int codePoint = displayStr.codeUnitAt(0);
        Random random = Random(codePoint);
        // 示例：使用随机颜色
        return Color.fromARGB(
          255,
          100 + random.nextInt(156), // 控制颜色明亮度
        ...
        return colorMap.containsKey(colorKey) ? colorMap[colorKey]! : const Color.fromARGB(255, 82, 82, 82);
      }
    }
